...
export const validateVehicle = (vehicle) => {
  ...
  VEHICLE_TYPES.REQUIRED_FIELDS.forEach(field => {
    if (!vehicle[field]) {
      errors.push(`Campo ${field} es requerido`);
    }
  });
  
  VEHICLE_TYPES.NUMERIC_FIELDS.forEach(field => {
    if (vehicle[field] !== undefined && (isNaN(vehicle[field]) || vehicle[field] < 0)) {
...
export const normalizeVehicle = (vehicle) => {
  ...
  return {
    ...
    cilindrada: Number(vehicle.cilindrada) || 0,
